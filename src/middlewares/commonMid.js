...
// ==> Authorization function for creating a book
...
const authorize = async function (req, res, next) {
    try {
        ...
        if (userId !== req.decodedToken.userId) return res.status(403).send({ status: false, message: "Provide your own userId to create a book." })
        next()
    } catch (err) {
...
// ==> Authorization function for updating and deleting a book
...
const authForParams = async function (req, res, next) {
    try {
        ...
        if (book.userId.toString() !== req.decodedToken.userId) return res.status(403).send({ status: false, message: "You are not authorized to access this book." })
        next()
    } catch (err) {
        return res.status(500).send({ status: false, message: err.message })
    }
...
module.exports = { authenticate, authorize, authForParams }  // --> exporting the functions